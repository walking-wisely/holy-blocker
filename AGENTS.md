...
| `machine-learning`          | Python                        | **Baseline evaluation only, no fine-tuning.** Ships `Falconsai/nsfw_image_detection` (pretrained ViT, Apache-2.0) as-is and measures its false-positive rate on a fully synthetic UI/text-screenshot corpus (code editors, chat, documents, terminals, spreadsheets, forms) via `synth_ui.py` + `eval.py`. No threshold is picked; a score distribution and threshold sweep are reported instead (300 synthetic images: FPR 9.7% @ 0.20, 2.3% @ 0.50). See `docs/components/machine-learning/plan.md` for details and what's deliberately deferred (tile-max geometry, recall, real screenshots, fine-tuning) |
