...
| `machine-learning` | Python | Full pipeline: fine-tuning loop, dataset, eval metrics (FP/FN, ROC-AUC, PR-AUC, miss-budget operating points), single-class corpus harness + release guardrail (`corpus.py`, `gate.py`), ONNX + int8 + TFLite exports all verified end to end. `holy-blocker-extract` converts a corpus into non-viewable feature vectors; `holy-blocker-finetune` is resumable; `holy-blocker-score` regenerates the frozen experiment holdouts with per-medium AUC. Full-unfreeze model at 0.9832 ROC-AUC on the common holdout, superseding the 0.9766 unfreeze-3 model — see `docs/components/machine-learning/results.md` and `docs/components/machine-learning/experiments/`. Python <3.14. Corpora and artifacts gitignored |
