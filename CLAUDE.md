...
| `apps/mobile` | Kotlin / Android | AccessibilityService text path MVP — policy core + ScanGate + overlay + onboarding done; text-policy wired in over UniFFI; SettingsGuard blocks the accessibility/App Info screens that would remove the guard, with a timed in-app disable as the exit path; device admin active, so uninstall is refused until it is deactivated; all verified on an android-36 arm64 emulator. Plain Device Admin only — never design around Device Owner. **Known gap:** some settings screens harvest an empty node tree and go unguarded (the device-admin list among them) — see `docs/components/mobile/backlog.md` item 1. Empty-harvest + split-screen window resolution, then tamper log, foreground service, VpnService/MediaProjection |
