...
| `apps/mobile` | Kotlin / Android | AccessibilityService text path MVP — policy core + ScanGate + overlay + onboarding done; text-policy wired in over UniFFI; SettingsGuard blocks the accessibility/App Info screens that would remove the guard, with a timed in-app disable as the exit path; device admin active, so uninstall is refused until it is deactivated; all verified on an android-36 arm64 emulator. Plain Device Admin only — never design around Device Owner. The empty-harvest gap is closed — the device-admin list is guarded, via `isAccessibilityTool` against Android 14's `accessibilityDataSensitive`, which is also a Play policy declaration (see `docs/components/mobile/plan.md` step 7). `WindowResolver` handles split-screen window choice for the guard, which waits rather than acting on an unfocused match — **the two-pane case is unit-tested only, not device-verified**. **Text only: imagery is not covered at all** (no MediaProjection, no `image-sandbox`), and in split screen the content path still harvests the focused pane while blaming the event's package — `docs/components/mobile/backlog.md` item 8. Next work is organised as workstreams in `docs/components/mobile/plan.md` (shared window model, then guard, content, image path); read that before extending either path, since tamper resistance and content interception need opposite rules for an unfocused window |
...
| `machine-learning` | Python | Full pipeline: fine-tuning loop, dataset, eval metrics (FP/FN, ROC-AUC, PR-AUC, miss-budget operating points), single-class corpus harness + release guardrail (`corpus.py`, `gate.py`), ONNX + int8 + TFLite exports all verified end to end. `holy-blocker-extract` converts a corpus into non-viewable feature vectors; `holy-blocker-finetune` is resumable; `holy-blocker-score` regenerates the frozen experiment holdouts with per-medium AUC; `holy-blocker-anime` subsamples `deepghs/anime_dbrating` over ranged reads into a drawn training supplement, and `mixture_plan` composes the anime experiment's three arms (baseline / addition / ablation control) while holding the validation half frozen. The anime subsampling experiment is **run and inconclusive** — adding the anime data lowered drawn AUC 0.9604 → 0.9526, so it is not adopted; per-medium routing is the follow-up. Full-unfreeze model at 0.9832 ROC-AUC on the common holdout, superseding the 0.9766 unfreeze-3 model — see `docs/components/machine-learning/results.md` and `docs/components/machine-learning/experiments/`. Python <3.14. Corpora and artifacts gitignored |
