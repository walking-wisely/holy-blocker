...
| Window / page titles and URLs fed to scoring | `apps/desktop/src/main/ipc-handlers.ts` — `ScanEvent.windowTitle` (l.9) held in the last-500 ring buffer (l.20); `apps/mobile/.../policy/TextPolicy.kt` — `PolicySource.BROWSER_TITLE`/`BROWSER_URL` (l.22-23) scored by `evaluate`, never stored | ? — a title or URL is content-adjacent metadata; on a sexual-content page it reveals that interest, but standing depends on the page (marked `?`, not `Y`) | Desktop bounded and stated: `RING_BUFFER_CAP = 500`, in-memory, never persisted (`ipc-handlers.ts:20`). Android: no store found — `PolicySource` values are enum labels consumed and dropped | none |
