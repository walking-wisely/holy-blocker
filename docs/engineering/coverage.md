...
`Where` records whether the code is on `master` or stranded on a branch. Work described as done in
`CLAUDE.md` but living on an unmerged branch has not shipped, and this column is why the distinction
is tracked here rather than in the status table.
...
| Blocked app kept visible after a block | `WindowSuppression` | **Covered** | branch | `NSRunningApplication.hide()`; block only, never warn; protected set never hidden |
...
| Content in native fullscreen | overlay | **Unverified** | branch | Never observed over native-fullscreen video |
| Content on a second display | overlay | **Unverified** | branch | Reconcile across display connect/disconnect never observed |
| DRM/HDCP-protected video | — | **Uncovered** | — | Captures as black by design. Platform limit, not a defect |
| HTTPS page content via the proxy | Layer 1 `mitm-proxy` + `CATrust` | **Partial** | `master` | Firefox 152 renders an HTTPS page through the proxy with the CA installed. Pinned clients fail closed, as expected |
...
| A protected user who is a local admin | — | **Uncovered by design** | — | Open product decision in `docs/decisions/content-interception.md`; `assess()` reports `weakened` and does not refuse to run |
...
| Explicit imagery on screen | MediaProjection capture → classifier | **Not built** | branch `feat/mobile-screen-capture` | Capture and gating are done; the sink is a counter. The classifier needs the LiteRT backbone + `packages/classifier-head`, neither of which exists |
| Plaintext DNS to a blocked domain | `NetworkGuardService` | **Covered** | branch `feat/mobile-vpn-service` | `scripts/smoke-test-vpn.sh` on an android-36 emulator |
...
| A forged DNS answer from an off-path attacker | `NetworkGuardService.ask()` | **Uncovered — live defect** | branch | Unconnected `DatagramSocket`, no transaction-ID or question check; the answer is wrapped and written into the TUN |
| SNI / IP-level filtering | `net-shield` | **Not built** on Android | — | Needs a userspace TCP stack; widening the route first black-holes the device |
...
| Disabling the guard via `adb`, guest account, or an unrecognised OEM screen | `GuardStatusService` + `TamperLog` | **Covered (recorded, not prevented)** | `master` | Records what it cannot prevent, by design |
| Revoking the VPN from Settings | — | **Recorded, not prevented** | branch | `NETWORK_GUARD_REVOKED`. The VPN pane is deliberately not in `SettingsProfiles` — identifiers are dumped from a device, never inferred |
...
| An empty, truncated, or deleted `blocklist.txt` | — | **Uncovered** | branch | An open guard with no signal |
