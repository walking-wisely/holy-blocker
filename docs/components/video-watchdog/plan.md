...
The design rationale and pipeline context live in [network-pipeline.md](../../architecture/network-pipeline.md) (Phase 5).
...
- Socket drop and stream severing — actively killing the TCP connection to the browser on a Block verdict requires coordination with the proxy's connection table; this is deferred and replaced with a log warning for now (see [network-pipeline.md](../../architecture/network-pipeline.md) Phase 5.3).
